...
- show the dashboard immediately without requiring a sign-in;
- keep administrator setup and sign-in available as an optional flow;
- show stream lists, filters, lifecycle actions, viewer counts, and reactions;
- display RTMP publishing details to stream owners and allowed account roles;
...
| Route | Purpose |
| --- | --- |
| `/` | Public stream list and guest stream creation |
| `/streams/:streamId` | Public stream details and player; owners can manage the stream |
| `/auth` | Optional administrator setup and sign-in |
...
Before joining a live stream, the page asks for a display name and then joins
the dashboard WebSocket with that name and a generated viewer ID. It receives
viewer-count, reaction, and stream-status events for the selected stream.
Reconnect attempts use delays from one to fifteen seconds. The hook
